...
Enemy::Enemy() {
	EX = Red.GetEnemyX();
	EY = Red.GetEnemyY();
	EXC = Red.GetEnemyX() / MAP_SIZE;
	EYC = Red.GetEnemyY() / MAP_SIZE;
	DFLG = false;
}
...
void Enemy::EnemyController() {
	...
	if (EX <= 435 && EY == 405 && VectorFlg == LEFT) {
		EX = 700;
	}
	if (EX >= 700 && EY == 405 && VectorFlg == RIGHT) {
		EX = 435;
	}

	if (EX <= 429 && EY == 405 && VectorFlg == LEFT) {
		EX += 1;
	}
	if (EX <= 429 && EY == 405 && VectorFlg == RIGHT) {
		EX -= 1;
	}
	if (EX >= 851 && EY == 405 && VectorFlg == LEFT) {
		EX += 1;
	}
	if (EX >= 851 && EY == 405 && VectorFlg == RIGHT) {
		EX -= 1;
	}
	...
	Red.SetMoveDir(VectorFlg);

}

float Enemy::DistanceSqrf(const float t_x1, const float t_y1, const float t_x2, const float t_y2) {
	float dx = t_x2 - t_x1;
	float dy = t_y2 - t_y1;
...
bool Enemy::CheckHit(const BOX& t_box, const CIRCLE& t_circle) {
	bool nResult = false;
...
bool Enemy::CheckHit(const BOX1& t_box, const CIRCLE& t_circle) {
	bool nResult = false;
...
bool Enemy::CheckHitBOX(const BOX& t_direA, const BOX1& t_direB)
{
